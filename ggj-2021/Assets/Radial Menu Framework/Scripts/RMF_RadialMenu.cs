...
public class RMF_RadialMenu : MonoBehaviour
{
  ...
  void Update()
  {
    ...
    if (!useGamepad)
    {
      float deltaX = Input.mousePosition.x - rt.position.x;
      float deltaY = Input.mousePosition.y - rt.position.y;
      rawAngle = Mathf.Atan2(deltaY, deltaX) * Mathf.Rad2Deg;
      rawRadius = Mathf.Sqrt(deltaX * deltaX + deltaY * deltaY);
    }
    else
    {
    ...
    if (angleOffset != 0 && useLazySelection)
    {
      ...
      {
        RectTransform buttonXform = elements[index].button.gameObject.GetComponent<RectTransform>();
        float btnHalfWidth = buttonXform.sizeDelta.x / 2;
        float btnHalfHeight = buttonXform.sizeDelta.y / 2;
        float btnRadius = Mathf.Sqrt(btnHalfWidth * btnHalfWidth + btnHalfHeight * btnHalfHeight);
        float offsetRadius = buttonXform.localPosition.y;
        float sectionRadius = offsetRadius + btnRadius;

        if (rawRadius <= sectionRadius)
        {
          //Select it.
          selectButton(index);
  ...
  private void unselectButton(int i)
  {
    ...
    {
      elements[i].unHighlightThisElement(pointer); //Select this one
      previousActiveIndex = i;
    }
  }
